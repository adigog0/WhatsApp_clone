...
const ChatUsersCard = ({ user, active,selectChatUserHandler }: Iprops) => {
    ...
    return (
        ...
        onClick={()=>selectChatUserHandler(user)}
            className={`max-h-16 shadow-sm shadow-slate-400 flex min-h-[10%] ${
                user.user_id === active ? "bg-slate-300" : "bg-slate-50"
            }  rounded-md p-1 m-1 cursor-pointer overflow-auto hover:bg-slate-300 items-center ${user.user_email === user_email ? "hidden":"block"}`}
        >
            <img className="rounded-full h-12" src={user.user_picture ?? EmptyProfile } alt="" />
            <div className=" p-1 flex-auto w-full overflow-auto ml-2">
                <div className="flex justify-between w-full">
                    <span className="font-medium">{user.user_name}</span>
                    {user.date && <span className="text-sm"> 00:00</span>}
                </div>
